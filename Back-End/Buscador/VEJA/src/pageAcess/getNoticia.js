...
async function extractNoticia(link, verificaData) {
  ...
  const tags = Array.from(document.querySelectorAll(".article-tags li")).map((tag) => tag.textContent.trim());
  ...
  function extractContent(articleBody) {
   ...
   if (element.tagName.toLowerCase() === "p")  {
       ...
       if(propaganda){
         ...
         if (propaganda.tagName.toLowerCase() === "a")  {
          ...
          if (propaganda.tagName.toLowerCase() === "b")  {
                let texto = propaganda.textContent.trim();
                if (texto.startsWith("BAIXE")||texto.startsWith("IOS")||texto.startsWith("ANDROID")) {
                  // O primeiro texto começa com 
                 return;
                }
          } 
      ...
      if (element.classList.contains("infogram-embed") ||element.classList.contains("instagram-media")|| element.classList.contains("twitter-tweet")) {
